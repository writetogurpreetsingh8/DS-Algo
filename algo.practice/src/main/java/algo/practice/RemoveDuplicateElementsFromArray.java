 ...
 * 
 * @author Tony.Stark
 * uses count sort algo to find the duplicate and eleminate them
 * from array... TC O(N) , stack O(1) and aux space O(k) for temp array
 */
public class RemoveDuplicateElementsFromArray {

	
	private static void removeDuplicateElement(int  input[]){
		...
		System.out.println("invalid input array.....");
			...
			/*for(int i=0;i<temp.length;i++){
				...
				if(temp[i]>0){
				list.add(i); // here we have uniqueValues
				}
...
return max;
	...
	public static void main(String[] args) {
		...
		RemoveDuplicateElementsFromArray.removeDuplicateElement(input);
